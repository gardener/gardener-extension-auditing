...
IMAGE                       := europe-docker.pkg.dev/gardener-project/public/gardener/extensions/auditing
...
.PHONY: generate
generate: $(CONTROLLER_GEN) $(EXTENSION_GEN) $(CRD_REF_DOCS) $(HELM) $(KUSTOMIZE) $(YQ)
	@go mod download
...
generate-in-docker:
	docker run --rm -it -v $(PWD):/go/src/github.com/gardener/gardener-extension-auditing golang:1.26.2 \
