...
import (
	...
	admissioncmd "github.com/gardener/gardener-extension-auditing/pkg/admission/cmd"
	auditinginstall "github.com/gardener/gardener-extension-auditing/pkg/apis/auditing/install"
)

const admissionName = "auditing-admission"

var log = logf.Log.WithName("gardener-extension-auditing-admission")

// NewAdmissionCommand creates a new command for running an auditing admission controller.
func NewAdmissionCommand(ctx context.Context) *cobra.Command {
	var (
		...
		mgrOpts  = &extensionscmdcontroller.ManagerOptions{
			...
			WebhookCertDir:     "/tmp/admission-auditing-cert",
		}
	...
	cmd := &cobra.Command{
		Use: "gardener-extension-auditing-admission",

		RunE: func(_ *cobra.Command, _ []string) error {
			verflag.PrintAndExitIfRequested()

			log.Info("Starting auditing-admission", "version", version.Get())
			...
			auditinginstall.Install(mgr.GetScheme())
